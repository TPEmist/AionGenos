...
# ─────────────────── Idempotency helpers (v2, --skip resume safety) ───────────────────
# Each Step 5/6/7/8 task guards on a completion marker so that after a mid-
# pipeline crash, `--skip N` re-runs only tasks that HAVE NOT already
# produced their expected artifact. Never touches an already-completed arm.

# Remote adapter check — return 0 if adapter_config.json exists on server.
remote_adapter_done() {
  local path=$1
  ssh -o BatchMode=yes "$REMOTE_HOST" "test -f $REMOTE_ROOT/$path/adapter_config.json" 2>/dev/null
}

# Remote GGUF check — return 0 if adapter.gguf exists on server.
remote_gguf_done() {
  local path=$1
  ssh -o BatchMode=yes "$REMOTE_HOST" "test -f $REMOTE_ROOT/$path" 2>/dev/null
}

# Local collect check — return 0 if data/100ep-d11-${prot}/ has >= N replay JSONs.
local_collect_done() {
  local out_dir=$1
  local expect_n=$2
  [ -d "$out_dir" ] || return 1
  local got=$(find "$out_dir" -type f -name "*.json" 2>/dev/null | wc -l)
  [ "$got" -ge "$expect_n" ]
}

# Helper for skipping a step because its artifact already exists.
skip_done() {
  local step=$1; local reason=$2
  echo
  echo "════════ Step $step ════════"
  echo "  ✓ ALREADY DONE — $reason (skipping, safe to --skip through)"
}
...
for arm in "${ARMS[@]}"; do
  ...
  if [ "$DRY_RUN" -eq 0 ] && [ "$SKIP_TO" -le 5 ] && remote_adapter_done "$SFT_CKPT/final_adapter"; then
    skip_done "5.${arm}" "$SFT_CKPT/final_adapter/adapter_config.json exists"
    continue
  fi
...
for arm in "${ARMS[@]}"; do
  ...
  if [ "$DRY_RUN" -eq 0 ] && [ "$SKIP_TO" -le 6 ] && remote_adapter_done "$KTO_CKPT/final_adapter"; then
    skip_done "6.${arm}" "$KTO_CKPT/final_adapter/adapter_config.json exists"
    continue
  fi
...
for arm in "${ARMS[@]}"; do
  ...
  SFT_CKPT="$CHECKPOINTS_DIR/${arm}/sft_A"
  KTO_CKPT="$CHECKPOINTS_DIR/${arm}/kto_B"
  SFT_GGUF="data/lora_gguf/d11_${arm}_sft/adapter.gguf"
  KTO_GGUF="data/lora_gguf/d11_${arm}_kto/adapter.gguf"
  if [ "$DRY_RUN" -eq 0 ] && [ "$SKIP_TO" -le 7 ] \
       && remote_gguf_done "$SFT_GGUF" && remote_gguf_done "$KTO_GGUF"; then
...
collect_done() {
  # Return 0 if any logs/d11_${prot}_*.log contains "Collect loop execution complete."
  local prot=$1
  # shellcheck disable=SC2144
  for f in logs/d11_${prot}_*.log; do
    [ -f "$f" ] || continue
    if grep -q "Collect loop execution complete" "$f"; then return 0; fi
  done
  return 1
}
...
for prot in "${PROTOCOLS[@]}"; do
  ...
  # Idempotency gate: if a prior invocation's log for this protocol already
  # contains "Collect loop execution complete", skip both reload and collect
  # (reload is expensive and student is still on the correct adapter iff we
  # haven't touched it since — but a full retest is safer than assuming, so
  # keep gate on collect only, allowing reload to run again cheaply).
  if [ "$DRY_RUN" -eq 0 ] && [ "$SKIP_TO" -le 8 ] && collect_done "$prot"; then
    skip_done "8.${prot}.collect" "logs/d11_${prot}_*.log shows collect completion"
    continue
  fi
