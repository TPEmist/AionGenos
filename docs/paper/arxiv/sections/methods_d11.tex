% Converted from d11_methods_draft.md — mechanical markdown->LaTeX (format only).
% Source draft H1 title: D11 Methods — draft v1
% Note: this file contains the D11 Methods body plus Appendix A1 (amendment
% chain) and Appendix A2 (filter audit trail); they are NOT duplicated elsewhere.

\tmlrnote{Mechanical translation of the frozen pre-registration. Every design
...
to a fixed round budget, where a \emph{round} is one VLM reasoning step
followed by an inverse-kinematics servo to the emitted target. All
evaluation runs use \texttt{--freeze\_level} (no curriculum auto-advance) so
every arm sees the identical task distribution.

\subsection*{3.2 Teacher and memory pipeline}
...
state-aware ranking) and injects them as a PAST\_LESSONS preamble. The
...
the C\_retrieval protocol later reuses at student inference.

\subsection*{3.3 Students, arms, and the 2$\times$2 factorial}

Students are LoRA adapters (rank-16, $\alpha$=32, dropout 0.05) on the same
Gemma-4-31B-it base, targeting the language-model attention and MLP
projections only (\texttt{self\_attn.\{q,k,v,o\}\_proj}, \texttt{mlp.\{gate,up,down\}\_proj});
the vision tower is untouched. Each arm is trained in two stages on the
\emph{same} pinned trajectory pool: SFT (stage 4-A, 992 desirable
...
$\beta$=0.1, auto-balanced class weights) with the frozen SFT adapter as the
...
logged to \texttt{training\_meta.json} for audit.

The four training arms form a 2$\times$2 factorial over two binary factors ---
...
\toprule
 & no gist & with gist \\
\midrule
\textbf{no native thought} & A\_action\_only & D\_gist \\
\textbf{with native thought} & A\_ctrl\_rat & B\_main \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection*{3.4 Evaluation protocols}

Five inference protocols are evaluated, 100 episodes each, all at the
same task with \texttt{--freeze\_level}:
...
\midrule
A\_action\_only & own & none \\
A\_ctrl\_rat & own & none \\
B\_main & own & none \\
D\_gist & own & none \\
C\_retrieval & A\_ctrl\_rat's & frozen teacher buffer snapshot \\
\bottomrule
\end{tabular}
\end{table}

C\_retrieval is \emph{not} a fifth trained arm: it reuses A\_ctrl\_rat's
adapter weights and re-attaches the frozen retrieval buffer as an
inference-time PAST\_LESSONS preamble. Two contrasts therefore follow,
and we keep them distinct (a distinction the results section makes
central):
\begin{itemize}
\item \textbf{Identical-weights} (C\_retrieval vs A\_ctrl\_rat): same adapter,
  retrieval toggled --- the clean parametric-vs-contextual contrast.
  Not pre-registered as primary (reported exploratory).
\item \textbf{Registered protocol} (C\_retrieval vs B\_main = T4): each arm its
  own adapter, so this varies both adapter and protocol; pre-registered
  and confirmatory (Amendment 8).
\end{itemize}

The C\_retrieval buffer is read-only during evaluation; a tree-hash gate
...
episode seed = 4500 + episode index), so episode \emph{k} starts from the
same initial world configuration across protocols --- a paired design
(Amendment 11).

\subsection*{3.5 Pre-registered hypotheses and tests}

Registered before any adapter trained (pre-reg \S2--\S4, refined by
Amendments 9--10):

\begin{itemize}
\item \textbf{T1 (primary)}: B\_main $-$ A\_action\_only. Rejection: $\Delta$ SR $\ge$ +10 pp
  AND significant, two-sided, $\alpha$=0.020. A graded reading within this
  budget line: T1-strong ($\ge$+10 pp) and T1-weak ($>$0, significant at
  $\alpha$=0.010). Explicit falsification clause on the memory-in-weights
  claim.
\item \textbf{T1a}: B\_main $-$ A\_ctrl\_rat, two-sided, $\alpha$=0.020.
\item \textbf{T4}: C\_retrieval $-$ B\_main, two-sided, $\alpha$=0.010.
\item \textbf{T3}: B\_main SR $\ge$ 0.7 $\times$ pooled memory-teacher SR (=36.2\%),
  one-sided.
\item Family-wise $\alpha$=0.060 (Amendment 10, non-uniform allocation).
\item \textbf{R1 $\Delta$X probe} (mechanism, pre-registered per-arm prediction matrix,
  Amendment 9 \S9.4): round-1 target X minus initial EE X, per arm,
  ...
  choice (H\_language vs H\_behavior).
\end{itemize}

\subsection*{3.6 Analysis rules (locked post-collection, pre-p-value)}

The two-timestamp discipline (\S4.1): the analysis rules below were
locked (Amendment 14) after collection but before any p-value, with the
raw counts frozen out of the test-selection decision.

\begin{enumerate}
\item \textbf{Pairing-integrity gate}: for each episode index, a 5-way
   \texttt{allclose} ($\varepsilon$=1e-4 m) on the initial configuration; on failure, a
   mechanical fallback (below) --- no per-contrast discretion.
\item \textbf{Test selection}: gate pass $\rightarrow$ McNemar (paired) primary; gate fail
   $\rightarrow$ two-proportion z primary. Binary, all-or-nothing.
\item All pairwise tests two-sided (pre-reg \S4, never altered).
\end{enumerate}
...
\subsection*{3.7 Data curation and its honest history}
...
\emph{coherence}, not \emph{correctness}, and were demoted to advisory flags;
only the vacuity rule retains (structurally guaranteed, Amendment 13)
zero drop authority under the \texttt{flags\_only\_a6} policy actually used, so
...
\section*{Appendix A1 --- Pre-registration amendment chain}
...
\# & Date & Change & Anchor \\
\midrule
1 & 07-06 & Sample-count control arm (B\_matched) added & pre-reg \S12 \\
2 & 07-06 & Filter escalation rule (agreement $<$85\% $\rightarrow$ LLM judge) & --- \\
...
6 & 07-07 & Coherence filter $\rightarrow$ advisory flags (v-final 992+1810) & 9b0a9da \\
7 & 07-08 & Pre-flight checklist (length align, seed, env reset) & bc524b8 \\
8 & 07-08 & Cross-arm target hygiene + 2$\times$2 + C\_retrieval protocol & 1a44d61 / a7bada2 \\
9 & 07-08 & T1 threshold, R1 probe branches, thought-leak, rename & 503496b \\
10 & 07-08 & B\_matched removed (confounder dissolved), $\alpha\rightarrow$T1a & bf2dbb5 \\
11 & 07-08 & Paired design $\rightarrow$ McNemar; env\_seed\_base=4500; SHA gate & bfa7cc6 \\
12 & 07-08 & Canonical filenames, D\_gist eval slot, SHA-gate timing & ae196b5 / 0e26443 \\
13 & 07-08 & Step-2 row-count sentinel; structural \texttt{flags\_only\_a6} & 9f5bec1 / cd1eb94 \\
14 & 07-13 & Analysis-time decision lock (gate, fallback, two-sided) & 00be6d5 / 73e712b \\
\bottomrule
\end{tabular}
\end{table}

\tmlrnote{(Amendments 5--13 filed 07-07/07-08 in the pre-flight window; SHAs are
the frozen commits. Full text of each is in the pre-registration
document.)}

\tmlrnote{Counts shown are as pinned at each amendment's date; the final
...
\section*{Appendix A2 --- Filter audit trail}
...
\texttt{flags\_only\_a6} policy used in the final pipeline is a structural
